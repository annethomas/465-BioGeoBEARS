...
plot_BioGeoBEARS_results=function(results_object, analysis_titletxt=NULL, 
                                  ...
                                  axiscex = NULL, axispadj= NULL, plotsplits=TRUE, 
                                  plotlegend=FALSE, legend_ncol=NULL, legend_cex=1, 
                                  cornercoords_loc="manual", tr=NULL, tipranges=NULL, 
                                  if_ties="takefirst", pie_tip_statecex=0.7, 
                                  juststats=FALSE, xlab="Millions of years ago", 
                                  root.edge=TRUE, colors_list_for_states=NULL, 
                                  skiptree=FALSE, show.tip.label=TRUE, tipcol="black", 
                                  dej_params_row=NULL, plot_max_age=NULL, skiplabels=FALSE,
                                  plot_stratum_lines=TRUE, include_null_range=NULL, 
                                  plot_null_range=FALSE, simplify_piecharts=FALSE, 
                                  tipboxes_TF=TRUE, tiplabel_adj=c(0.5), no.margin=FALSE, 
                                  xlims=NULL, ylims=NULL)
{
  ...
  if (skiptree != TRUE)
  {
    ...
    if(!is.null(axiscex)){
      axis(side=1,  at=xaxis_ticks_locs, label=xticks_desired,cex.axis=axiscex)
    }else{
      axis(side=1,  at=xaxis_ticks_locs, label=xticks_desired)
    }
    # Plot the title
    if(!is.null(axispadj)){
      mtext(text=xlab, side=1, line=2, cex=titlecex,padj = axispadj)
    } else{
